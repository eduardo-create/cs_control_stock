...
export default function ReportePromosPage() {
  const { token } = useAuth();
  ...
  async function exportar() {
    ...
    try {
      ...
      a.href = url;
      a.download = `promos_mas_vendidas_${filtro.desde || 'inicio'}_a_${filtro.hasta || 'fin'}.xlsx`;
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-5 text-slate-900">
      <div className="rounded-2xl border border-slate-200 bg-white shadow p-5">
        <h2 className="text-2xl font-bold mt-1">Promociones más vendidas</h2>
        <p className="text-sm text-slate-500 mt-1">Lista las promociones vendidas en el rango indicado, sin detalle de productos.</p>
      </div>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white shadow overflow-x-auto">
        <div className="min-w-[860px]">
          <div className="grid grid-cols-[320px_150px_150px_120px_220px] gap-3 px-4 py-3 text-xs font-semibold uppercase tracking-tight text-slate-500 border-b border-slate-100 whitespace-nowrap">
            <span>Promoción</span>
            <span>Promo ID</span>
            <span>Cantidad</span>
            <span>Tickets</span>
          ...
          <div className="divide-y divide-slate-100">
            ...
            {rows.map((r, idx) => (
              <div key={idx} className="grid grid-cols-[320px_150px_150px_120px_220px] gap-3 px-4 py-3 items-center text-sm text-slate-800 whitespace-nowrap">
                <div className="font-semibold">{r.promocion || `Promo ${r.promocion_id}`}</div>
                <div className="text-slate-600 text-[13px]">{r.promocion_id || '—'}</div>
                <div className="font-semibold">{formatNumber(r.cantidad_total)}</div>
                <div>{formatNumber(r.tickets)}</div>
                <div className="font-semibold">{formatMoney(r.total_bruto)}</div>
              </div>
            ))}
          </div>
        </div>
